...
def main():
    ...
    # save the dataframes
    with open("data/LMS_3.csv", "w", encoding="utf-8", newline="") as f:
        lms_graded[1].to_csv(f, index=False)
    subprocess.run(
        ["vd", "-f", "csv", "-"],
        input=lms_graded[1].to_csv(index=True),
        text=True,
    )

    ann = tools.lms_to_announced(announced_grades_path, df_orig, "data/announced_3.xls")
